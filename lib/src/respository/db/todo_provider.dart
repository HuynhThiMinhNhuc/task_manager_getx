          ...
          onCreate: (Database db, int version) async {
            await db.execute('''
              create table ${AppConstant.tableTodo} ( 
                ...
                ${AppConstant.columnStatus} integer not null)
              ''');
          },
        ),
      );
...
create table ${AppConstant.tableTodo} (
  ...
  ${AppConstant.columnTitle} text not null,
  ${AppConstant.columnStatus} integer not null,
  ${AppConstant.collumnDescription} text,
  ${AppConstant.columnDueDate} text,
  ${AppConstant.createdAt} text,
  ${AppConstant.updatedAt} text)
''');
        },
      );
    }
  ...
  Future<TodoItem?> getTodo(int id) async {
    List<Map> maps = await db.query(
      AppConstant.tableTodo,
      columns: [
        AppConstant.columnId,
        AppConstant.columnStatus,
        AppConstant.columnTitle,
        AppConstant.collumnDescription,
        AppConstant.columnDueDate,
        AppConstant.createdAt,
        AppConstant.updatedAt,
      ],
      where: '${AppConstant.columnId} = ?',
      whereArgs: [id],
    );
    if (maps.isNotEmpty) {
